...
export default function DysonRemonts({
  idBase = 'services',
  title,
  bodyHTML,
  href,
  ...
  return (
    <article className={`${s.halfCard} ${s.halfStack}`} aria-labelledby={`${bid}-title`}>
      <h3 id={`${bid}-title`} className={s.cardTitle}>{title}</h3>

      <div className={`${s.stackVisual} ${s.withHalo}`} aria-hidden="true">
        <div className={s.visualPad}></div>
        <Image
      ...
      <a className={s.cardLink} href={href}>{linkLabel}</a>
    </article>
  );
}

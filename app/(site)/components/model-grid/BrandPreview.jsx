...
export default function BrandPreview({ brandSlug, brandName, items = [], total = 0, href }) {
  if (!items || items.length === 0) return null;

  return (
    <section id={`brand-${brandSlug}`} className={layout.section} aria-labelledby={`brand-${brandSlug}-h2`}>
        ...
        <div className={g.grid}>
          {items.map((d) => (
            <div className={g.gridItem} key={`${d.brandSlug}:${d.slug}`}>
              <ModelCard device={d} baseHref={href} />
            </div>
          ))}
        </div>
      </div>

// app/(site)/(catalog)/datoru-remonts/[brand]/[device]/page.jsx
...
// Find a laptop device by brand + slug in datoru category
...
// Normalize price strings (copied from phone page)
...
async function buildPriceListItems(modelSlug) {
  ...
  // Helper to map a service to a PriceList row with optional Firestore data
  const toRow = (base, data = null) => {
    ...
    // Keep existing rule: hide line if price === null (explicitly unapplicable)
    if (raw === null) return null;

    // If no Firestore data -> empty string => "pēc pieprasījuma" in PriceList
    const priceText =
      data == null
        ? ''
        : typeof raw === 'number'
    ...
    return {
      ...
      priceTo,
      popular: false,
      // FIX: ensure datoru service URLs
      href: base.slug ? `/datoru-remonts/${base.slug}` : base.href,
    };
  };

  // ✅ Fallback: no Firestore docs yet -> show all datoru services as "pēc pieprasījuma"
  if (snap.empty) {
  ...
  // Existing: merge Firestore modelServices with catalog
...
// Build FAQ items + LD, preferring COMPUTER/LAPTOP context if it exists
...
export async function generateMetadata({ params }) {
  ...
  const title =
    d?.metaTitle ||
    (d ? `${d.name} remonts Rīgā | iLab` : `${brandLabel} datoru remonts | iLab`);

  const description =
    d?.metaDescription ||
    (d
      ? `${d.name} remonts Rīgā: akumulatora un displeja nomaiņa, atjaunošana pēc šķidruma bojājumiem, profilakse, tastatūra, touchpad. Ātra diagnostika, godīgas cenas un garantija.`
  ...
  return {
    title,
    description,
    alternates: { canonical: `/datoru-remonts/${brandSlug}/${slug}` },
  };
}
...
export default async function Page({ params }) {
  ...
  const modelPath = `/datoru-remonts/${brandSlug}/${d.slug}`;
  const provider = buildProvidersFromLocations(); // Domina + Spice
  ...
  const breadcrumbsLd = buildBreadcrumbsLd([
    ...
    {
      name: `${brandLabel} datoru remonts`,
      url: abs(`/datoru-remonts/${brandSlug}`),
    },

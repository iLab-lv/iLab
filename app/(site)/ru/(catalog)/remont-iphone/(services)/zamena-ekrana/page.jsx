import IphoneScreenServicePage from '@site/(catalog)/iphone-remonts/(services)/ekrana-maina/IphoneScreenServicePage';
...
const strings = {
  ...
  heroBodyHtml:
    '<p><strong>Замена экрана iPhone в Риге</strong> в сервисе iLab - качественные дисплеи, быстрая диагностика и <strong>гарантия до 1 года</strong>. Часто замену экрана выполняем в тот же день.</p>',
  ...
  serviceDescription:
    'Замена дисплея iPhone в Риге. Бесплатная диагностика и гарантия до 1 года.',
  ...
  headerLead:
    'Меняем экран iPhone при трещинах, полосах, пятнах, отсутствии изображения или проблемах с сенсором. До ремонта проводим диагностику, согласовываем стоимость и после замены выдаём гарантию до 1 года.',
  headerCtaLabel: 'Смотреть цены',

  applyAria: 'Записаться на ремонт',

  processName: 'Замена экрана iPhone',
  processDescription:
    'Как проходит замена экрана iPhone в iLab: диагностика, согласование цены, замена дисплея, проверка и гарантия.',
  processSteps: [
    {
      name: 'Диагностика',
      text: 'Проверяем экран, сенсор и работу iPhone.',
    },
    {
      name: 'Цена и сроки',
      text: 'Согласовываем стоимость и срок замены экрана до начала работы.',
    },
    {
      name: 'Замена экрана',
      text: 'Меняем дисплей и проверяем подключение новой детали.',
    },
    {
      name: 'Проверка',
      text: 'Тестируем изображение, сенсор, Face ID и основные функции.',
    },
    {
      name: 'Гарантия',
...
export default async function Page({ searchParams }) {
  ...
  const data = await getIphoneScreenServiceData({
    selectedModel,
  });

  return (
    <>
      <JsonLd id="iphone-screen-service-ru-jsonld" data={data.jsonLd} />

      <IphoneScreenServicePage locale={locale} {...data} />
    </>
  );
}

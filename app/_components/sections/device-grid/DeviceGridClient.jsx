...
export default function DeviceGridClient({
  ...
  return series.map((group) => {
    ...
    const matchingDevices = group.devices.filter(
      (device) => selectedYear === null || Number(device.year) === selectedYear
    );
    const visibleSlugs = new Set(
      (isExpanded ? matchingDevices : matchingDevices.slice(0, initialLimit)).map(
        (device) => device.slug
      )
    );
    const canExpand = group.devices.length > initialLimit;

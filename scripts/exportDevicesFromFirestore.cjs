/* scripts/exportDevicesFromFirestore.cjs
 *
 * Exports Firestore collection: devices
 * into a local JSON file.
 *
 * Usage:
 *   node scripts/exportDevicesFromFirestore.cjs
 *
 * Optional env:
 *   EXPORT_PATH=tmp/devices.export.json
...
const EXPORT_PATH =
  process.env.EXPORT_PATH || 'tmp/devices.export.json';
...
async function run() {
  console.log('=== Export devices from Firestore ===');
  ...
  console.log(`Exported ${items.length} devices`);
